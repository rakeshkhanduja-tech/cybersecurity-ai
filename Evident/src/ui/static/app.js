    ...
    while (true) {
        try {
            ...
            if (res.ok) {
                ...
                // Update progress bar
                const logLength = (data.log && data.log.length) ? data.log.length : 1;
                const progress = Math.min(95, (logLength / 5) * 100); // Assuming 5 steps for 100%
                if (loadingBar) {
                ...
                if (msgEl) {
                    msgEl.textContent = data.step || data.details || 'Initializing...';
                }
...
// ─── Data Plugs View Loader ──────────────────────────────────────────────────

async function loadDataPlugsView() {
    const plugsList = document.getElementById('plugs-list');
    if (!plugsList) return;

    plugsList.innerHTML = '<div class="loading" style="text-align:center; padding: 40px; color: var(--text-secondary);">Loading connectors...</div>';

    try {
        const [connRes, activeRes] = await Promise.all([
        ...
        if (allPlugs.length === 0) {
            plugsList.innerHTML = '<div class="placeholder-text" style="text-align:center; padding: 40px; opacity: 0.5;">No connectors configured.<br>Add connector definitions to <code>data_plugs.json</code>.</div>';
            return;
        }

        renderPlugs(allPlugs, activePlugIds);
    } catch (e) {
        console.error('[DataPlugs] Error loading view:', e);
        plugsList.innerHTML = '<div class="error" style="text-align:center; padding: 40px;">Failed to load connectors. Is the agent running?</div>';
    }
}
...
function switchView(viewId) {
    ...
    if (viewId === 'data-plugs') loadDataPlugsView();
    if (viewId === 'signals') {
        loadSources();
        loadStats();
    }
...
function setupModals() {
    const dataPlugsPane = document.getElementById('pane-data-plugs');
    ...
    // Sidebar: Data Plugs
    const btnDataPlugs = document.getElementById('btn-data-plugs');
    if (btnDataPlugs) {
        btnDataPlugs.addEventListener('click', () => {
            switchView('data-plugs');
        });
    }
    ...
    document.querySelectorAll('.close-pane').forEach(btn => {
        btn.addEventListener('click', (e) => {
            const pane = e.target.closest('.context-pane');
...
function updateStorageVisibility() {
    const modeEl = document.getElementById('settings-source-mode');
    const schemaEl = document.getElementById('settings-schema');
    if (!modeEl || !schemaEl) return;

    const mode = modeEl.value;
    const schema = schemaEl.value;

    const pathGroup = document.getElementById('data-path-group');
    const storageGroup = document.getElementById('storage-type-group');
    const pathInput = document.getElementById('settings-data-path');

    if (mode === 'sample') {
        if (storageGroup) storageGroup.style.display = 'none';
        if (pathGroup) pathGroup.style.display = 'block';
        if (pathInput) {
            pathInput.value = schema === 'ocsf' ? 'Evident/data/sample_ocsf' : 'Evident/data/sample';
        }
    } else {
        if (storageGroup) storageGroup.style.display = 'block';
        if (pathGroup) pathGroup.style.display = 'none';
        if (window.toggleStorageFields) toggleStorageFields();
    }
}
...
document.addEventListener('DOMContentLoaded', () => {
    const modeEl = document.getElementById('settings-source-mode');
    const schemaEl = document.getElementById('settings-schema');
    if (modeEl) modeEl.addEventListener('change', updateStorageVisibility);
    if (schemaEl) schemaEl.addEventListener('change', updateStorageVisibility);
});
// Settings Modal Logic: Clear fetch context when provider changes
const _settingsProviderEl = document.getElementById('settings-provider');
if (_settingsProviderEl) {
    _settingsProviderEl.addEventListener('change', () => {
        const modelSelect = document.getElementById('settings-model');
...
const _fetchModelsBtn = document.getElementById('btn-fetch-models');
if (_fetchModelsBtn) {
    _fetchModelsBtn.addEventListener('click', async () => {
...
const _saveSettingsBtn = document.getElementById('btn-save-settings');
if (_saveSettingsBtn) {
    _saveSettingsBtn.addEventListener('click', async () => {
        const provider = document.getElementById('settings-provider').value;
        const apiKey = document.getElementById('settings-api-key').value;
        ...
        if (storageType === 'azure') {
            ...
            storageConfig.data_path = document.getElementById('storage-local-path').value;
        }
...
function populateStorageFields(s) {
    ...
    } else if (s.storage_type === 'local') {
        const localPathEl = document.getElementById('storage-local-path');
        if (localPathEl) localPathEl.value = s.data_path || '';
    }
    toggleStorageFields();
}
...
// --- Data Plugs Logic (Modernized) ---
// Legacy functions loadDataPlugsPane, renderConnectorList, etc. have been removed.
// Data Plugs are now handled via setupSidePanes() and renderPlugs().
...
const NODE_COLORS = {
    ...
    role: '#a5d6a7',
    // OCSF Class mappings
    authentication: '#ce93d8',
    'security finding': '#ff5252',
    'compliance finding': '#80cbc4',
    'vulnerability finding': '#ff5252',
    'device inventory info': '#4fc3f7',
    'account change': '#ce93d8',
    'network activity': '#ffb74d',
    default: '#90a4ae'
};
...
function findCrossSourceRelationships(item) {
    ...
    // For OCSF, prioritize raw_data fields
    const target = item.raw_data ? { ...item, ...item.raw_data } : item;

    const selfFields = [];

    function extractFields(obj, prefix = '') {
        for (const [k, v] of Object.entries(obj)) {
            if (IGNORE_FIELDS.has(k) || v == null) continue;
            const fullKey = prefix ? `${prefix}.${k}` : k;

            if (Array.isArray(v)) {
                const vals = v.map(x => typeof x === 'object' ? null : String(x).trim().toLowerCase())
                    .filter(x => x && x.length > 1 && !NULL_VALS.has(x));
                if (vals.length > 0) selfFields.push({ field: fullKey, values: vals });
            } else if (typeof v === 'object') {
                extractFields(v, fullKey);
            } else {
                const val = String(v).trim().toLowerCase();
                if (val.length > 1 && !NULL_VALS.has(val)) {
                    selfFields.push({ field: fullKey, values: [val] });
                }
            }
        }
    }

    extractFields(target);
    ...
    for (const [sourceName, entities] of Object.entries(_graphDataCache)) {
        for (const other of entities) {
            ...
            for (const { field: selfField, values: selfVals } of selfFields) {
                // Check flat and nested fields for the 'other' entity
                const otherTarget = other.raw_data ? { ...other, ...other.raw_data } : other;
                ...
                function findMatch(obj) {
                    for (const [k, v] of Object.entries(obj)) {
                        ...
                        if (Array.isArray(v)) {
                            for (const x of v) {
                                if (typeof x !== 'object' && selfVals.includes(String(x).toLowerCase())) return { field: k, value: x };
                            }
                        } else if (typeof v === 'object') {
                            const m = findMatch(v);
                            if (m) return m;
                        } else {
                            if (selfVals.includes(String(v).toLowerCase())) return { field: k, value: v };
                        }
                    }
                    return null;
                }

                const match = findMatch(otherTarget);
                if (match) {
                    const label = relLabel(selfField.split('.').pop()); // Use simple field name for label

                    results[groupKey].push({
                        relatedItem: other,
                        sourceName,
                        fieldOnSelf: selfField,
                        fieldOnOther: match.field,
    ...
    agents.forEach(agent => {
        ...
        card.innerHTML = `
            ...
            <div class="agent-footer" style="flex-direction: column; gap: 12px; align-items: stretch;">
                ...
                <div style="background: rgba(0,0,0,0.3); border: 1px solid rgba(79, 195, 247, 0.2); padding: 6px 10px; border-radius: 6px; font-family: 'Fira Code', monospace; font-size: 0.7rem; color: #4fc3f7; display: flex; align-items: center; gap: 8px;">
                    <span style="opacity: 0.6;">MCP:</span>
                    <span>${agent.mcp_endpoint}</span>
                </div>
                ` : ''}
...
// --- Infrastructure View Logic ---

function toggleInfraSections() {
    const type = document.querySelector('input[name="store-type"]:checked').value;
    const sqliteFields = document.getElementById('infra-sqlite-fields');
    const postgresFields = document.getElementById('infra-postgres-fields');
...
async function loadInfraView() {
    try {
        const res = await fetch('/api/config/infra');
        const data = await res.json();

        // Config Store
        const cs = data.config_store;
        const typeRadio = document.querySelector(`input[name="store-type"][value="${cs.type}"]`);
        if (typeRadio) typeRadio.checked = true;

        document.getElementById('infra-sqlite-path').value = cs.sqlite.path;
        document.getElementById('infra-sqlite-abs-path').textContent = `Full Path: ${data.sqlite_absolute_path}`;

        document.getElementById('infra-pg-host').value = cs.postgres.host;
        document.getElementById('infra-pg-port').value = cs.postgres.port;
        document.getElementById('infra-pg-db').value = cs.postgres.database;
        document.getElementById('infra-pg-user').value = cs.postgres.user;
        document.getElementById('infra-pg-pass').value = ''; // Don't show password

        // Knowledge Store
        const gdb = data.graph_db;
        document.getElementById('infra-neo-uri').value = gdb.uri;
        document.getElementById('infra-neo-db').value = gdb.database;
        document.getElementById('infra-neo-user').value = gdb.user;
        document.getElementById('infra-neo-pass').value = '';

        // Vector Store
        const vdb = data.vector_db;
        document.getElementById('infra-vector-path').value = vdb.path;
        document.getElementById('infra-vector-collection').value = vdb.collection_name;
        document.getElementById('infra-vector-model').value = vdb.embedding_model;

        toggleInfraSections();

    } catch (e) {
        console.error('[INFRA] Error loading config:', e);
        showToast('Failed to load infrastructure configuration', 'error');
    }
}

async function saveInfraConfig() {
    const saveBtn = document.getElementById('btn-save-infra');
    ...
    const payload = {
        config_store: {
            type: document.querySelector('input[name="store-type"]:checked').value,
            sqlite: { path: document.getElementById('infra-sqlite-path').value },
            postgres: {
                host: document.getElementById('infra-pg-host').value,
                port: document.getElementById('infra-pg-port').value,
                database: document.getElementById('infra-pg-db').value,
                user: document.getElementById('infra-pg-user').value,
                password: document.getElementById('infra-pg-pass').value
            }
        },
        graph_db: {
            uri: document.getElementById('infra-neo-uri').value,
            database: document.getElementById('infra-neo-db').value,
            user: document.getElementById('infra-neo-user').value,
            password: document.getElementById('infra-neo-pass').value
        },
        vector_db: {
            path: document.getElementById('infra-vector-path').value,
            collection_name: document.getElementById('infra-vector-collection').value,
            embedding_model: document.getElementById('infra-vector-model').value
        }
    };
    ...
    try {
        ...
        if (res.ok) {
            showToast('Infrastructure configuration saved successfully', 'success');
        } else {
            const err = await res.json();
            showToast(`Save failed: ${err.message}`, 'error');
        }
    } catch (e) {
        showToast('System error while saving configuration', 'error');
    } finally {
...
async function initConfigDB() {
    if (!confirm('This will attempt to initialize the database schema. Tables will be created if they do not exist. Continue?')) return;

    const initBtn = document.getElementById('btn-init-db');
    initBtn.disabled = true;
    initBtn.textContent = 'Initializing...';

    try {
        const res = await fetch('/api/config/init-db', { method: 'POST' });
        const data = await res.json();

        if (res.ok) {
            showToast('Database schema initialized - Readiness confirmed', 'success');
        } else {
            showToast(`Initialization failed: ${data.message}`, 'error');
            console.error('[DB] Init error:', data);
        }
    } catch (e) {
        showToast('Connection error during initialization', 'error');
    } finally {
        initBtn.disabled = false;
        initBtn.textContent = '🛠️ Initialize Database Schema';
    }
}

// Hook up events
document.addEventListener('DOMContentLoaded', () => {
    const saveInfraBtn = document.getElementById('btn-save-infra');
    if (saveInfraBtn) saveInfraBtn.addEventListener('click', saveInfraConfig);

    const initDbBtn = document.getElementById('btn-init-db');
    if (initDbBtn) initDbBtn.addEventListener('click', initConfigDB);
});

// --- Data Plugs View Events ---
function setupDataPlugsView() {
    // Save plugs button
    const savePlugsBtn = document.getElementById('btn-save-plugs');
    if (savePlugsBtn) {
        savePlugsBtn.addEventListener('click', saveActivePlugs);
    }

    // Save button in the new connector config modal
    const saveConfigBtn = document.getElementById('btn-save-connector-config');
    if (saveConfigBtn) {
        saveConfigBtn.addEventListener('click', saveConnectorConfig);
    }

    // Search filter for connector cards
    const plugSearch = document.getElementById('plug-search');
    if (plugSearch) {
        plugSearch.addEventListener('input', (e) => {
            const term = e.target.value.toLowerCase();
            document.querySelectorAll('.connector-card').forEach(card => {
                const nameText = card.querySelector('h4') ? card.querySelector('h4').textContent.toLowerCase() : '';
                const descText = card.querySelector('p') ? card.querySelector('p').textContent.toLowerCase() : '';
                card.style.display = (nameText.includes(term) || descText.includes(term)) ? '' : 'none';
            });
        });
    }
}

async function renderPlugs(allPlugs, activePlugIds) {
    const plugsList = document.getElementById('plugs-list');
    if (!plugsList) return;
    ...
    allPlugs.forEach(plug => {
        const isActive = activePlugIds.has(plug.id);
        const card = document.createElement('div');
        card.className = `connector-card ${isActive ? 'active' : ''}`;
        card.dataset.id = plug.id;

        card.innerHTML = `
            <div class="status-badge">${isActive ? 'Active' : 'Disabled'}</div>
            <div class="connector-icon">${plug.icon || '🔌'}</div>
        ...
        card.onclick = () => openConnectorConfig(plug.id);
        plugsList.appendChild(card);
    });
}

/**
 * Opens the configuration modal for a specific connector
 */
async function openConnectorConfig(connectorId) {
    console.log(`[Config] Opening modal for: ${connectorId}`);
    const modal = document.getElementById('modal-connector-config');
    if (!modal) return;

    try {
        // Fetch all connectors and stored configs
        const [connRes, configRes] = await Promise.all([
            fetch('/api/connectors'),
            fetch('/api/connectors/config')
        ]);
        const connData = await connRes.json();
        const storedConfigs = await configRes.json();

        const plugMeta = (connData.connectors || []).find(p => p.id === connectorId);
        const savedData = (storedConfigs.configs || {})[connectorId];

        if (!plugMeta) {
            showToast('Connector specification not found', 'error');
            return;
        }

        // Set ID, Icon, Name
        document.getElementById('connector-config-id').value = connectorId;
        document.getElementById('connector-config-icon').textContent = plugMeta.icon || '🔌';
        document.getElementById('connector-config-name').textContent = `Configure ${plugMeta.name}`;

        // Set Active Toggle
        const activeToggle = document.getElementById('connector-config-active');
        activeToggle.checked = savedData ? !!savedData.is_active : false;

        // Build Dynamic Form Fields
        const fieldsArea = document.getElementById('connector-config-fields');
        fieldsArea.innerHTML = '';

        const params = plugMeta.parameters || [];
        const currentConfig = savedData ? (savedData.config || {}) : {};

        if (params.length === 0) {
            fieldsArea.innerHTML = '<p style="color: #888; font-size: 0.85rem; text-align: center; padding: 20px;">This connector requires no additional parameters.</p>';
        } else {
            params.forEach(param => {
                const labelText = param.split('_').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' ');
                const isPassword = /key|secret|token|pass/i.test(param);
                const value = currentConfig[param] || '';

                const fieldGroup = document.createElement('div');
                fieldGroup.className = 'form-group';
                fieldGroup.innerHTML = `
                    <label style="display: block; margin-bottom: 8px; font-size: 0.8rem; color: #aaa;">${labelText}</label>
                    <input type="${isPassword ? 'password' : 'text'}"
                           class="connector-param-input"
                           data-key="${param}"
                           value="${value}"
                           placeholder="Enter ${labelText}..."
                           style="width: 100%; background: rgba(0,0,0,0.3); border: 1px solid var(--border-color); border-radius: 8px; color: #fff; padding: 10px 14px; font-size: 0.9rem;">
                `;
                fieldsArea.appendChild(fieldGroup);
            });
        }

        modal.classList.add('show');
    } catch (e) {
        console.error('[Config] Error opening modal:', e);
...
async function saveConnectorConfig() {
    const connectorId = document.getElementById('connector-config-id').value;
    const isActive = document.getElementById('connector-config-active').checked;
    const saveBtn = document.getElementById('btn-save-connector-config');
    const originalText = saveBtn.textContent;

    // Collect all parameters
    const config = { is_active: isActive };
    document.querySelectorAll('.connector-param-input').forEach(input => {
    ...
    try {
        ...
        if (res.ok) {
            ...
            document.getElementById('modal-connector-config').classList.remove('show');

            // Hard refresh of views to show new status and data
            // Small delay ensures DB commits are reflected in subsequent API calls
            setTimeout(async () => {
                await loadDataPlugsView();
                await loadSources();
                if (typeof updateStats === 'function') updateStats();
            }, 500);

            // Trigger intelligence rebuild if connector was activated
            if (isActive) {
                console.log('[Config] Triggering background rebuild...');
                fetch('/api/rebuild', { method: 'POST' }).catch(e => console.error('Silent rebuild fail:', e));
            }
        } else {
            const err = await res.json();
            showToast(`Save failed: ${err.message}`, 'error');
...
function updateCardActive(card, isActive) {
    const dot = card.querySelector('.status-dot');
    const statusText = card.querySelector('.connector-status span:last-child');
    if (isActive) {
