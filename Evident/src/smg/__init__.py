...
from src.smg.node_builder import SecurityNodeBuilder
from src.smg.relationship_builder import SecurityRelationshipBuilder
from src.smg.neo4j_store import Neo4jGraphStore
from src.smg.mock_store import MockGraphStore
    ...
    def __init__(self, use_mock: bool = None):
        ...
        if use_mock:
            ...
            try:
                ...
                print("[OK] Using Neo4j graph store")
            except Exception as e:
                print(f"[ERROR] Failed to initialize Neo4j: {e}")
                print("[WARN] Falling back to mock graph store")
                self.graph_store = MockGraphStore()
        
        # Initialize builders
        if config_loader.load_config().ingestion.schema_preference == "ocsf":
            from src.smg.ocsf_node_builder import OCSFNodeBuilder
            from src.smg.ocsf_relationship_builder import OCSFRelationshipBuilder
            self.node_builder = OCSFNodeBuilder()
            self.relationship_builder = OCSFRelationshipBuilder()
        else:
