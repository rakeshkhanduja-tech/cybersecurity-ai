...
def get_storage(storage_cfg: dict):
    ...
    if stype == "azure":
        from src.connectors.storage.azure_storage import AzureStorage
        return AzureStorage(storage_cfg)
    if stype == "aws_s3":
        from src.connectors.storage.aws_storage import AwsS3Storage
        return AwsS3Storage(storage_cfg)
    if stype == "gcs":
        from src.connectors.storage.gcs_storage import GcsStorage
        return GcsStorage(storage_cfg)

    from src.connectors.storage.local_storage import LocalStorage
