...
from src.rag.embedder import SecurityDocumentEmbedder
...
class RAGEngine:
    ...
    def __init__(self, vector_store: Optional[VectorStore] = None):
        # Always read fresh config — not the stale module-level app_config
        cfg = config_loader.load_config()

        # Initialize vector store
        if vector_store is None:
            self.vector_store = VectorStore(
                persist_directory=cfg.vector_db.path,
                collection_name=cfg.vector_db.collection_name
            )
        else:
            self.vector_store = vector_store

        # Initialize embedder based on current schema preference
        if cfg.ingestion.schema_preference == "ocsf":
            from src.rag.ocsf_embedder import OCSFDocumentEmbedder
            self.embedder = OCSFDocumentEmbedder()
        else:
            self.embedder = SecurityDocumentEmbedder()

        # Configuration
        self.top_k = cfg.agent.retrieval_top_k
        self.max_context_length = cfg.agent.max_context_length
